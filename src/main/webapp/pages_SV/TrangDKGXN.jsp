...
<body>
			  ...
			  <div class="container">
			   ...
			   <form action="<%=request.getContextPath()%>/SV/dk_DKGXN" method="post">
				    <div class="container text-left">
				        ...
				        &emsp;&emsp;&emsp;&emsp;
				        <input type="submit" class="btn btn-success" value="Đăng ký" onclick="return checkRegistration()">
				    </div>
				</form>
			   <br>
				   ...
				   <table class="table table-bordered">
				    ...
				    <tbody>
				     ...
				     <c:forEach var="dkgxn" items="${listDKGXN}">
				      <tr class="dkgxn" data-maLoaiGiay="<c:out value='${dkgxn.maLoaiGiay}'/>" data-trangThai="<c:out value='${dkgxn.trangThai}'/>">
				       ...
				       <td class="col7"><a href="delete_GXN?id=<c:out value='${dkgxn.id}'/>" id="deleteLink" style="display: none;">Xóa</a></td>
	...
	<script>
	    function checkRegistration() {
	        // Lấy giá trị maLoaiGiay từ select box
	        var maLoaiGiay = document.getElementById('loaigiay').value;
	
	        // Lấy danh sách các maLoaiGiay đã đăng ký
	        var registeredMaLoaiGiay = []; // Thay đổi thành danh sách thực tế của bạn
	
	        // Lấy danh sách maLoaiGiay từ các phần tử dkgxn
	        var dkgxnElements = document.querySelectorAll('.dkgxn');
	        dkgxnElements.forEach(function(element) {
	            if (element.getAttribute('data-trangThai') === 'Chưa duyệt' || element.getAttribute('data-trangThai') === 'Đã duyệt') {
	                registeredMaLoaiGiay.push(element.getAttribute('data-maLoaiGiay'));
	            }
	        });
	
	        if (registeredMaLoaiGiay.includes(maLoaiGiay)) {
	            alert('Bạn đã đăng ký giấy xác nhận này rồi');
	            return false;
	        } else {
	            return true;
	        }
	...
	<script>
	  window.onload = function() {
	    var rows = document.getElementsByClassName('dkgxn');
