...
<body>
	...
	<div class="wrapper">
		...
		<div class="sidebarnew">
			<ul class="menu">
				<li><a href="<%= request.getContextPath()%>/Ctsv/listTB" class="special-link">Trang chủ</a></li>
		...
		<div class="main-content">  
			...
			<div style="width: 100%;" class ="tables-container">
				<div style="height: 360px; overflow: auto;">
					<table id="myTable" border="1" class="table" style="width: 99%; border-collapse: collapse; font-size: 20px;">                       
						<thead>
							<tr>
								<th>Tên dịch vụ</th>
								<th>Người gửi </th>
						...
						<tbody>
						   <c:forEach var="tb" items="${listTB}">
								<tr>
									<td>${tb.getTenDV()}</td>
									<td>${tb.getTenSV()}</td>  
									<td>${tb.getTGGui().toLocalDateTime().format(DateTimeFormatter.ofPattern('dd-MM-yyyy HH:mm:ss'))}</td>    
									<td >${tb.getTenKhoa()}</td>  
									<td >${tb.getTenKH()}</td>               
								</tr>							
							</c:forEach>
...
<script>
	...
	for (var i = 0; i < rows.length; i++) {
	    if (window.getComputedStyle(rows[i]).display === "table") {
	        rowCount++;
	    }
	}
	rowCount = rowCount-1;
