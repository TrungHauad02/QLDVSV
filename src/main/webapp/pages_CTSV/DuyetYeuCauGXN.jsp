...
<head>
	...
	<title>Duyệt yêu cầu giấy xác nhận</title>
...
<body>
	...
	%>
	<%
		XacNhanGXN gxn = (XacNhanGXN)request.getAttribute("gxn");
		String tengiay = gxn.getTenLoaiGiay();
	%>
	<!-- Phần header -->
    ...
    <div class="wrapper">
        ...
        <div class="sidebarnew">
            <ul class="menu">
                ...
                <li><a href="<%=request.getContextPath()%>/Ctsv/listgxn" class="special-link">Quản lý giấy xác nhận</a></li>
	    ...
	    <div class="main-content">
		        <table class="mytable">
			        ...
			        <tbody>
			            <tr>
                            <td>${gxn.getThoiGianDangKi().format(DateTimeFormatter.ofPattern('dd-MM-yyyy HH:mm:ss'))}</td>  
                            <td><%= gxn.getMssv() %></td> 	                            
			    ...
			    <form action="<%=request.getContextPath()%>/Ctsv/confirmacceptgxn?id=${gxn.getID()}" method="post">
				    <fieldset style="border:1px solid black; padding: 10px;">
						...
						<div style="display: flex; flex-direction: column;">
							...
							<input type="text" id="title" name="title" style="width: 100%; box-sizing: border-box; margin-bottom: 10px;" value="Yêu cầu  <%= tengiay%> đã được duyệt ">
							<span id="titleError" style="color: red;"></span> <!-- Thêm span để chứa thông báo -->
					
					       <div style="display: flex; justify-content: space-between;">
							    <div style="flex: 1; margin-right: 5px;">
							        <label for="email" style="font-weight: bold;">Email:</label>
							        <input type="text" id="email" name="email" value="<%= gxn.getMssv() %>@student.hcmute.edu.vn" style="width: 100%; box-sizing: border-box;" readonly>
							        <span id="emailError" style="color: red;"></span> <!-- Thêm span để chứa thông báo -->
							    </div>
							    <div style="flex: 2; margin-left: 5px;">
							        ...
							        <span id="contentError" style="color: red;"></span> <!-- Thêm span để chứa thông báo -->
							    </div>
							</div>
					
					        <div style="display: flex; justify-content: space-between; margin-top: 10px;">
					            <button type="submit" class="button" id="submitBtn">Duyệt</button>
					            <button type="button" class="button" id="cancelBtn" onclick="location.href='<%=request.getContextPath()%>/Ctsv/listgxn'">Hủy</button>
					        </div>
					    </div>
					</fieldset>
				</form>
