        ...
        try {
            switch (action) {
                ...
                case "/thongbao":
                	...
                	break;
                case "/delete_GXN":
                	deleteGXN(request,response);
                	break;
                case "/delete_GXNNN":
                	deleteGXNNN(request, response);
                	break;
                case "/delete_CTXH":
                	deleteCTXH(request, response);
            }
	...
	private void insert_dkGXN(HttpServletRequest request, HttpServletResponse response)
		    throws SQLException, ServletException, IOException, ParseException {
				...
				if(dkgxnDAO.insertDKGXN(TenDichVu,SoLuong,MaLoaiGiay,MSSV,ThoiGianDK,TrangThai))
				{
					...
					request.getSession().setAttribute("status", "insert_TC");
			    } else {
			        request.getSession().setAttribute("status", "insert_TB");
			    }
...
//				show_DKGXN(request, response);
	...
	private void deleteGXN(HttpServletRequest request, HttpServletResponse response)
		    throws SQLException, ServletException, IOException, ParseException {
				HttpSession session = request.getSession();
				String id = request.getParameter("id");
				if(dkgxnDAO.delete_GXN(id))
				{
					System.out.println("thanh cong");
					request.getSession().setAttribute("status", "delete_TC");
			    } else {
			        request.getSession().setAttribute("status", "delete_TB");
			    }
...
//				show_DKGXN(request, response);
	...
	private void deleteGXNNN(HttpServletRequest request, HttpServletResponse response)
		    throws SQLException, ServletException, IOException, ParseException {
				HttpSession session = request.getSession();
				String id = request.getParameter("id");
				if(dkxnnnDAO.delete_DKXNNN(id))
				{
					System.out.println("thanh cong");
					request.getSession().setAttribute("status", "delete_TC");
			    } else {
...
//				show_DKGXN(request, response);
	...
	private void deleteCTXH(HttpServletRequest request, HttpServletResponse response)
		    throws SQLException, ServletException, IOException, ParseException {
				HttpSession session = request.getSession();
				String id = request.getParameter("id");
				if(dkctxhDAO.delete_DKCTXH(id))
				{
					System.out.println("thanh cong");
					request.getSession().setAttribute("status", "cancel_TC");
			    } else {
			        request.getSession().setAttribute("status", "cancel_TB");
			    }
...
//				show_DKGXN(request, response);
    ...
    throws SQLException, ServletException, IOException {
		...
		svDAO.upDate_ttll_SV(idTk, std, email);
      
        RequestDispatcher dispatcher = request.getRequestDispatcher("/SV/info");
		dispatcher.forward(request, response);
    }
