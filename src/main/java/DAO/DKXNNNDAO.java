...
public class DKXNNNDAO {
	...
	private static final String Select_DKXNNN_MSSV = "SELECT dki_xn_nganh_nghe.*, lydo.NoiDung\r\n"
			+ "FROM dki_xn_nganh_nghe\r\n"
			+ "JOIN lydo ON dki_xn_nganh_nghe.MaLyDo = lydo.MaLyDo where MSSV= ? and dki_xn_nganh_nghe.Trangthai != 'Đã xóa' order by dki_xn_nganh_nghe.id";
	
	private static final String Delete_DKXNNN ="UPDATE dki_xn_nganh_nghe\r\n"
	...
	public List<DKXNNganhNghe> selectDKXNNN(String MSSV) {
        ...
        try (Connection connection = JDBCUtil.getConnection();
            ...
            while (rs.next()) {	
            	...
            	dkxnnn.add(new DKXNNganhNghe(id, TenDichVu, NamHoc, HocKy, MaLyDo, NDLyDo, MSSV_, ThoiGianDK, TrangThai));
            }
        } catch (SQLException exception) {
            HandleExeption.printSQLException(exception);
        }
	...
	public boolean delete_DKXNNN(String id) {
		boolean result = false;
        // Step 1: Establishing a Connection
        try (Connection connection = JDBCUtil.getConnection();
            // Step 2:Create a statement using connection object
            PreparedStatement preparedStatement = connection.prepareStatement(Delete_DKXNNN);) {
