...
public class DKCTXHDAO {
	...
	private static final String Select_DKCTXH_MSSV = "SELECT dki_tham_gia_ctxh.*, ctxh.TenHD, ctxh.Diem, ctxh.NoiDung\r\n"
			...
			+ "WHERE dki_tham_gia_ctxh.MSSV = ? and dki_tham_gia_ctxh.TrangThai != 'Đã hủy' order by dki_tham_gia_ctxh.id";
	
	private static final String Delete_DKCTXH ="UPDATE dki_tham_gia_ctxh\r\n"
	...
	public List<DKCTXH> selectDKCTXH_MSSV(String MSSV) {
        ...
        try (Connection connection = JDBCUtil.getConnection();
            ...
            while (rs.next()) {	
            	...
            	dkctxh.add(new DKCTXH(id, TenDichVu, MaHD, TenHD, ThoiGianDK, MSSV_, NDPhanHoi, TrangThai, Diem, NoiDung));
            }
        } catch (SQLException exception) {
            HandleExeption.printSQLException(exception);
        }
	...
	public boolean delete_DKCTXH(String id) {
		boolean result = false;
        // Step 1: Establishing a Connection
        try (Connection connection = JDBCUtil.getConnection();
            // Step 2:Create a statement using connection object
            PreparedStatement preparedStatement = connection.prepareStatement(Delete_DKCTXH);) {
